...
```{r}
#Instead of starting with a summary start with the goals and expectations of the
#study. Basically Charles will want to look at this and go "OK this is worth
#pursuing"
```
...
```{r}
#I'm not sure you should bother spliting the 2 months. I would just pool that
#togetherto create a more resolved network. n=2 isn't enough to count as
#replication really so you might as well just create a metaweb.
```
...
```{r}
#I bet this is more detail than Charles cares about but that's fine :)
```

# Costs
The Centre for Biodiversity in Guelph currently quotes US$1/sample. 
