...
```
<br>
This leads us to the final question: it seems logical to assume that the commonest species returned per sample, regardless of PCR replicate or primer, will be the spider whose abdomen we've mashed up. That is: if you extracted from a *Tegenaria domestica* and then used a load of different PCR primers and replicated them lots, you'd assume that for each well on the sequencing run your commonest species would be *Tegenaria domestica*. But is that true? 
<br>
In Table \@ref(tab:primer-spider-table) we can see that the two sets of amplicons per spider don't always agree on what the COMMONEST spider species found in the sample was. It'll be interesting to get the spider IDs from BOLD soon to help us understand these issues more.
<br>
```{r}
...
<br>
I'd interpret Table \@ref(tab:primer-spider-table) as showing the limitations of metabarcoding primers for specific taxonomic assignment. However it important to note that its possible for a well-extracted specimen to have more of a prey species' DNA sequenced than its own.<br>
One thing that its worth having in the backs of our (well, my) minds is that the cases where the two primers disagree could be because one of the primers has produced a read which couldn't be well-assigned, which would be possible to see in the quality score given to it by the classification software I used (RDP classifier). I used the minimum viable quality score which was recommended by a recent paper (I think 0.6), but it could be that thats too low to be sensible in our case. 

When using the minimum viable quality score, any sequences which are unable to be assigned to species level with a confidence above that threshold, would get assigned to the lowest taxonomic level which has a score exceeding the threshold (be it genus, family, order, etc).<br>
Figure \@ref(fig:fig-quality-hist) shows the distribution of the quality scores we have for our commonest spider sequence per sample, and we can see that most of the spider assignments were actually pretty good. When eyeballing the raw data, I can see that for some samples we sometimes have disagreements when the two 'best' matches are at 0.99 confidence. It'll be interesting to compare this data with the genuine identifications when they come from BOLD, as they'll provide some better information on what we can use as a viable score for assignments, and if that varies by PCR primer.

```{r fig-quality-hist, fig.cap= "Histogram showing quality scores of the best-matches for identifying each spider"}
